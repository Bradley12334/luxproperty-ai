...
/**
 * The live Full Brief price — Product "Full Brief" (prod_Ux3xSXVFlXSRC0), £149 GBP one-time.
 *
 * Repriced 2026-08-19 (£14.99 → £149). Stripe `unit_amount` is immutable, so this is a
 * NEW price object on the same product; the old price_1Tx9py… (£14.99) is archived. An
 * archived price still resolves for sessions already created against it, and
 * sessionIsFullBrief() falls back to metadata.kind, so in-flight buyers still get granted.
 */
const LIVE_FULLBRIEF_PRICE = "price_1U6A43P7AaxWnYG2QicP1vmv";
