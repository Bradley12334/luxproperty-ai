...
export function applySectorPolicy(stats, sectorContext, district, sectorTxCount) {
  ...
  const n = sectorTxCount ?? sectorContext.txCount;
  return withholdValuation(
    ...
    `No fair-value or opening-offer range is quoted for this address. Sales in ${sectorContext.sector} sit about ` +
      `${mag}% ${dir} than ${district} as a whole${n ? ` across ${n.toLocaleString()} recorded sales` : ""}, so a range ` +
      `anchored to the district median would point you roughly ${mag}% ${wrongWay}. ${sectorContext.sector}'s own median ` +
      `is stated above; this brief reports price trends at district level, so no sector-anchored range is offered in its ` +
      `place. A deliberate omission, not missing data — the district figures and the sector's median are both shown, and ` +
      `the most recent sales are listed with their addresses below.`,
  );
}
