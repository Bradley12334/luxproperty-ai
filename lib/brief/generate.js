...
import { withCachedTransactions, withCachedSource } from "./cache.js";
...
export async function generate(rawPostcode, opts = {}) {
  ...
  const stationsPromise = src("stations", () => fetchStations(location));
  const schoolsPromise = src("schools", () => fetchSchools(location));
  const amenitiesPromise = src("amenities", () => fetchAmenities(location));
  ...
  const { value: flood, fetchedAt: floodFetchedAt } = await floodPromise;
  ...
  const { value: stationsResult, fetchedAt: stationsFetchedAt } = await stationsPromise;
  ...
  const { value: tflResult } = await tflPromise;
  ...
  const { value: schoolsResult, fetchedAt: schoolsFetchedAt } = await schoolsPromise;
  const schools = buildSchoolsSection(schoolsResult, location, tier);

  // ── Local amenities (EXP) — awaits the concurrent Overpass fetch ────────────
  const { value: amenitiesResult, fetchedAt: amenitiesFetchedAt } = await amenitiesPromise;
  ...
  const { value: airQualityResult, fetchedAt: airQualityFetchedAt } = await airQualityPromise;
  ...
  const { value: crimeResult, fetchedAt: crimeFetchedAt } = await crimePromise;
  ...
  const { value: planningResult, fetchedAt: planningFetchedAt } = await planningPromise;
  ...
  const { value: propertyTypeResult, fetchedAt: propertyTypeFetchedAt } = await propertyTypePromise;
