...
export async function generate(rawPostcode, opts = {}) {
  ...
  let cacheLayer = "live";
  let txMeta = null;
  let stale = false; // serve-stale bridge: true when the tx set is a dated fallback, not live
  let staleAsOf = null; // epoch ms the stale set was originally fetched (→ meta.dataAsOf)
  ...
  try {
    ...
    staleAsOf = result.staleAsOf ?? null;
    txSet = result.value;
    txMeta = txSet.meta;
  ...
  let sections = gateSections([areaVerdict, ...evidenceSections], tier);

  // Serve-stale bridge: when the transaction set is a dated fallback (not live), stamp every
  // section DERIVED from it with `dataAsOf` so the client shows "Sold price data as of {date}"
  // — no silent staleness. All six are computed from the stale set (exec summary + preOffer
  // included, per spec). LOCKED sections are skipped (they show the upgrade stub, not data).
  if (stale && staleAsOf) {
    const dataAsOf = new Date(staleAsOf).toISOString();
    const STALE_KEYS = new Set([
      "executiveSummary", "pricesTrendNegotiation", "nearbySoldPrices",
      "streetPriceRanking", "soldPricesMap", "preOfferQuestions",
    ]);
    sections = sections.map((s) =>
      s && s.state !== "LOCKED" && STALE_KEYS.has(s.key) ? { ...s, dataAsOf } : s,
    );
  }
  ...
  return {
    meta: {
      ...
      cached,
      cacheLayer,
      // Serve-stale bridge: true + the fetch date when the tx set is a dated fallback (not live).
      stale,
      dataAsOf: stale && staleAsOf ? new Date(staleAsOf).toISOString() : null,
