...
// Time budget for the whole transaction fetch, kept safely under the Vercel Hobby
// 60s function ceiling so an overrun yields an UNAVAILABLE section, not a 504.
...
const COMING_SOON_SECTIONS = [
  ...
  { key: "comparableSales",    title: "Comparable sales & valuation",  minTier: "PRO" },
...
export async function generate(rawPostcode, opts = {}) {
  ...
  const sections = [pricesSection, ...COMING_SOON_SECTIONS];
  ...
  return {
    meta: {
      ...
      dataError: dataError ? { code: dataError.code } : null,
    },
    sections,
  };
}
