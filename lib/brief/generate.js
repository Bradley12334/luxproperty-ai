...
export async function generate(rawPostcode, opts = {}) {
  ...
  // council-tax fetch. Stamp duty is computed from the spine window median (Unit 2).
  const councilTaxResult = await councilTaxPromise;
  const buyingCosts = buildBuyingCostsSection(councilTaxResult, null, location, tier);

  const sections = [pricesSection, nearbySoldPrices, streetPriceRanking, soldPricesMap, floodClimate, stationsCommute, commuteCalculator, schools, amenities, broadband, airQuality, buyingCosts, ...COMING_SOON_SECTIONS];
