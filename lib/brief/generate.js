...
import { getSpine } from "./tx-source.js";
import { withCachedSource, shouldSkipUpstream, recordUpstreamSuccess, recordUpstreamFailure } from "./cache.js";
...
import { overpassFailShape } from "./overpass.js";
...
export async function generate(rawPostcode, opts = {}) {
  ...
  const overpassBundle = () =>
    (_overpassBundle ??= (async () => {
      ...
      // error (no Overpass fallback). The public-Overpass client has been REMOVED (it was
      // dead on every request path and reached a third-party mirror); recover from git if
  ...
  // ── The spine, under a time budget ──────────────────────────────────────────
  // One call, two possible sources (offline PPD aggregate or the legacy Land
  // Registry SPARQL scan), one normalised Spine out. Which source runs is decided
  // by TX_SOURCE / TX_AGG_DISTRICTS; no section below can tell the difference.
  let stats = null;
  let spine = null;
  let dataError = null;
  let cached = false;
  ...
  try {
    spine = await getSpine(location.outcode, FETCH_YEARS, {
      now,
      signal: controller.signal,
      sector: location.sector ?? null,
    });
    stats = spine.stats;
    cached = spine.cached;   // set by both spine constructors — no ?? needed
    cacheLayer = spine.layer;
    txMeta = spine.txSet?.meta ?? { count: spine.totalCount, source: spine.source, asOf: spine.asOf };
  } catch (err) {
    // A good resolve already happened, so we still return a payload — the price
    // section renders UNAVAILABLE carrying this reason.
    dataError = isBriefError(err)
      ? err
      : Object.assign(new Error(String(err?.message || err)), { code: "UPSTREAM_ERROR" });
    console.warn(`[brief] generate(${location.outcode}): spine failed — ${dataError.code}: ${dataError.message}`);
  } finally {
    clearTimeout(timer);
  }
  ...
  const pricesSection = stats
    ? buildPricesSection(stats, location, tier, spine)
    : {
  ...
  // on a data-altitude failure spine is null and they degrade to UNAVAILABLE like
  // the price section, never a silent gap.
  const nearbySoldPrices = spine
    ? buildNearbySoldPricesSection(spine, location, tier, stats?.medianPrice ?? null)
    : {
        key: "nearbySoldPrices",
        title: `Recent sales in ${location.outcode}`,
        minTier: "PRO",
        state: "UNAVAILABLE",
        note:
          dataError?.code === "UPSTREAM_ERROR"
            ? "Sold-price data could not be retrieved for this postcode right now (Land Registry did not respond within the time budget). Recent in-district sales are temporarily unavailable — try again shortly."
            : "Recent in-district sold prices are unavailable for this postcode.",
        data: null,
      };

  const streetPriceRanking = spine
    ? buildStreetRankingSection(spine, location, tier, stats?.medianPrice ?? null)
    : {
  ...
  // the section. On a data-altitude failure spine is null → UNAVAILABLE.
  let soldPricesMap;
  if (spine) {
    const recentPostcodes = [...new Set(selectRecent(spine).items.map((t) => t.postcode))];
    const centroids = await geocodePostcodes(recentPostcodes);
    soldPricesMap = buildSoldPricesMapSection(spine, location, tier, centroids);
  } else {
    soldPricesMap = {
      key: "soldPricesMap",
  ...
  const preOfferQuestions = buildPreOfferQuestionsSection(
    { stats, spine, floodData: floodClimate.data, crimeData: crimeBreakdown.data },
    location,
    tier,
  );
  ...
  return {
    meta: {
      ...
      outcodeOnly: location.outcodeOnly ?? false,
      // WHICH BUILD ANSWERED THIS. Five separate times today a report described
      // output that did not come from the deployment being discussed — a stale
      // preview, a harness fixture, a synthetic median. Without this there is no way
      // to settle "is the page wrong, or is the description of it wrong?" from the
      // response itself. Absent locally, which is honest: there is no commit serving.
      build: {
        commit: process.env.VERCEL_GIT_COMMIT_SHA || null,
        ref: process.env.VERCEL_GIT_COMMIT_REF || null,
        env: process.env.VERCEL_ENV || null,
      },
      // What actually produced the price figures, so "which path served this" is a
      // fact in the payload rather than an inference from asOf being null.
      spineSource: spine?.source ?? null,
      ward: location.ward,
