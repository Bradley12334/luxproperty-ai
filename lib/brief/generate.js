...
/**
 * Area Screening Verdict (EXP) — the one section still pending after Phase 2f.
 *
 * Per the BRIEF_SPEC verdict guardrail, the verdict must be SYNTHESISED from the
 * section outputs (never a standalone claim), which is Phase 3's work. Until then it
 * ships as its own explicit PENDING card (not the old generic "coming soon" grid, now
 * removed): titled, tier-badged, and honest that the Good fit / Mixed / Limited call is
 * derived from the sections below and lands in the next phase.
 */
const AREA_VERDICT_PENDING = {
  key: "areaVerdict",
...
export async function generate(rawPostcode, opts = {}) {
  ...
  const sections = [AREA_VERDICT_PENDING, executiveSummary, pricesSection, nearbySoldPrices, streetPriceRanking, soldPricesMap, floodClimate, stationsCommute, commuteCalculator, schools, amenities, broadband, airQuality, buyingCosts, rentalSnapshot, crimeBreakdown, neighbourhood, preOfferQuestions, planning, developmentTracker, rentalDemand];
