...
function gatePricesInternals(section, tier) {
  ...
  if (data.negotiation && !isEntitled(tier, "section.preOfferStrategy")) {
    data.negotiation = {
      ...
      // Carried through the tier gate deliberately. `withheld` explains why a range
      // is absent for reasons that have NOTHING to do with tier — too few sales, or
      // a sector the district median misdescribes. Dropping it here left an
      // unexplained blank, which reads as a load failure rather than a decision.
      // It is not a paid figure; it is the reason a figure is not being shown.
      withheld: data.negotiation.withheld ?? null,
    };
  }
