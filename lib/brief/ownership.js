...
/**
 * The distinct outcodes this account owns — the "My briefs" library list.
 * @param {string|null|undefined} userId
...
export async function listOwnedOutcodes(userId) {
  ...
  try {
    ...
    if (error) {
      console.warn(`[brief/ownership] listOwnedOutcodes error for ${id}: ${error.message} — returning [].`);
      return [];
    }
    // Distinct, preserving most-recent-first order.
    const seen = new Set();
    const out = [];
    for (const row of data ?? []) {
      const oc = normOutcode(row.outcode);
      if (oc && !seen.has(oc)) { seen.add(oc); out.push(oc); }
    }
    return out;
  } catch (err) {
    console.warn(`[brief/ownership] listOwnedOutcodes threw for ${id}: ${err?.message || err} — returning [].`);
    return [];
  }
}
