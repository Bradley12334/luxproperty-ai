 ...
 * £149 one-off, per-outcode permanent purchase).
 *
 ...
 * @param {string|null} [p.postcode] the buyer's TYPED full postcode (e.g. "SW1A 1AA"),
 *        stored for routing/display only. NULL for a bare-outcode purchase. NEVER the
 *        entitlement key — `outcode` remains that.
 ...
 */
export async function grantFullBrief({ userId, outcode, stripeSessionId, amountPaid = null, currency = null, postcode = null }) {
  ...
  // Display/routing only — a trimmed full postcode, or null (bare-outcode buy).
  const pc = (typeof postcode === "string" && postcode.trim()) ? postcode.trim() : null;
  ...
  const { error } = await supabase
    .from("brief_purchases")
    .insert({ user_id: id, outcode: oc, stripe_session_id: sid, amount_paid: amountPaid, currency, postcode: pc });
 ...
 * @param {string|null|undefined} userId
 * Each entry carries the originally-purchased full `postcode` (or null for a bare-outcode
 * buy / pre-migration row) so the library can link straight to the buyer's point brief.
 * @returns {Promise<Array<{ outcode: string, grantedAt: string, postcode: string|null }>>}  [] on absent input or any error.
 */
export async function listOwnedBriefs(userId) {
  ...
  try {
    const { data, error } = await supabase
      .from("brief_purchases")
      .select("outcode, granted_at, postcode")
      .eq("user_id", id)
    ...
    // Distinct by outcode, preserving most-recent-first order (keeps the newest purchase's
    // full postcode if a district was somehow bought twice).
    const seen = new Set();
    const out = [];
    for (const row of data ?? []) {
      ...
      if (oc && !seen.has(oc)) {
        seen.add(oc);
        const pc = (typeof row.postcode === "string" && row.postcode.trim()) ? row.postcode.trim() : null;
        out.push({ outcode: oc, grantedAt: row.granted_at, postcode: pc });
      }
    }
    return out;
  } catch (err) {
