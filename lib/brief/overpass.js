 ...
 *  fetchStations and the combined bundle so both produce byte-identical output. */
function parseStations(elements, lat, lng) {
  const raw = [];
...
function parseSchools(elements, lat, lng) {
  const raw = [];
  for (const el of elements) {
    const tags = el.tags || {};
...
/** Parse amenity elements → the four shaped groups. Shared by fetchAmenities and the
 *  combined bundle. Only shop/food/health/leisure-tagged elements are collected, so it
 *  runs unchanged over a combined element set (stations/schools are simply ignored). */
function parseAmenities(elements, lat, lng) {
  const supermarkets = [];
  const food = [];
  const health = [];
  const parks = [];
