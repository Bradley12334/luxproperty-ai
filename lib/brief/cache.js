...
const DEFAULT_TTL_MS = 45 * 24 * 60 * 60 * 1000; // 45d — Land Registry PPD covers 2016→
// present and is republished MONTHLY at most, so a district's sold-price history barely
// moves week to week. A 7d TTL forced needless full cold SPARQL re-scans, and on the
// observed EN-MASSE expiry it left every request re-scanning live → exposed to Land
// Registry latency (UPSTREAM_ERROR: generation budget exceeded) on warm instances. 45d
// keeps a district warm across several data cycles while still refreshing comfortably
// inside a monthly-to-quarterly cadence.
// DELIBERATELY DECOUPLED from SOURCE_TTL_MS (still 7d, below): flood/crime/air-quality/
// planning have shorter real freshness needs and are intentionally NOT changed here.
