...
export function classifySector(sector, districtMedian) {
  const none = {
    ...
    // Carried so the sector's own figure can be RENDERED, not just described. The
    // withholding copy promises the reader keeps this fact; without n and the band
    // there is nothing to state it with.
  ...
  if (sector.txCount >= SECTOR_MIN_FOR_SERVE) {
    return {
      ...
      ciLo: sector.ciLo, ciHi: sector.ciHi,
      divergencePct,
      errorPct,
      note:
