 ...
 *
 * PHASE 2a SCOPE — shape now, enforcement later:
 *   - The config below is complete (sections, depth variants, non-section quotas).
 *   - `isEntitled()` runs a REAL cumulative-tier check, so the payload builder is
 *     already wired through the enforcement path.
 *   - `accountTier()` is STUBBED to INV (unlock-all): every account is treated as
 *     Investor for now, so all sections — including the PRO pre-offer/negotiation
 *     section rendered in Phase 2a — resolve to full DATA. Real plan lookup
 *     (Stripe-linked account, per BRIEF_SPEC.md) replaces the stub in a later phase;
 *     only this one function changes.
 *
...
export const ENTITLEMENTS = Object.freeze({
  ...
  "quota.briefsPerMonth":       { minTier: "EXP", kind: "quota", label: "Briefs per month", quota: { EXP: 3, PRO: Infinity, INV: Infinity } },
});

/**
 * The account's tier at generation time.
 *
 * STUBBED (Phase 2a): always INV — unlock everything so the payload builder emits
 * full DATA for every section, including the PRO pre-offer/negotiation section.
 * Replace the body with the real Stripe-linked plan lookup in the gating phase;
 * nothing else in the pipeline needs to change.
 *
 * @param {unknown} [_account]  ignored while stubbed
 * @returns {Tier}
 */
export function accountTier(_account) {
  return "INV";
}
