 ...
 */
function deriveStats({ district, window, totalCount, median: windowMedian, ciLo, ciHi, perYear }) {
  ...
  // ── Below the window floor: the district has sales, but not enough for the
  //    median to be a price statistic. Say that, rather than printing one. ─────
  if (totalCount < WINDOW_MIN_FOR_MEDIAN) {
    return unavailableStats(district, window, trend, {
      level: "none",
      note:
        `Only ${totalCount} recorded sale${totalCount === 1 ? "" : "s"} in ${district} across ` +
        `${endYear - startYear + 1} years \u2014 too few for a median that means anything ` +
        `(its own margin of error would be wider than the price differences you are trying to judge). ` +
        `Individual sales are still listed where available.`,
      totalCount,
