 ...
 */
export function buildFloodClimateSection(flood, location, tier, fetchedAt = null) {
  const base = {
    key: "floodClimate",
  ...
  const warnings = formatWarnings(activeWarnings, warningsChecked, fetchedAt);
...
function formatWarnings(warnings, checked, fetchedAt = null) {
  if (!checked) {
    return { active: false, checked: false, stale: false, fetchedAt: fetchedAt ?? null, count: 0, items: [], text: "The live flood-warning service couldn’t be reached in time, so current warnings couldn’t be confirmed — check the up-to-the-minute picture at gov.uk/check-flood-risk." };
  }
  // Fix 1, req (c): if the flood evidence was SERVED FROM CACHE (fetched >10 min ago),
  // the live warnings were NOT re-checked this refresh — a real-time signal must never
  // present a cached picture as the current one. Stamp the fetched-at date + say so.
  const ageMs = fetchedAt == null ? 0 : Math.max(0, Date.now() - fetchedAt);
  const stale = ageMs > 10 * 60 * 1000;
  const asOf = fetchedAt ? new Date(fetchedAt).toISOString().slice(0, 10) : null;
  if (!warnings.length) {
    const text = stale
      ? `As of ${asOf}, no flood warnings or alerts were in force here — this brief served cached flood evidence and did not re-check the live service this refresh. Confirm the current picture at gov.uk/check-flood-risk.`
      : "No flood warnings or alerts were in force here at the time this brief was generated.";
    return { active: false, checked: true, stale, fetchedAt: fetchedAt ?? null, count: 0, items: [], text };
  }
  const text = stale
    ? `${warnings.length} flood ${warnings.length === 1 ? "warning/alert was" : "warnings/alerts were"} in force here as of ${asOf} — cached evidence, not re-checked this refresh. Check the live picture at gov.uk/check-flood-risk.`
    : `${warnings.length} active flood ${warnings.length === 1 ? "warning/alert is" : "warnings/alerts are"} in force near these coordinates right now — check the live picture at gov.uk/check-flood-risk.`;
  return {
    active: true,
    checked: true,
    stale,
