...
export function buildPricesSection(stats, location, tier, spine = null) {
  ...
  // The sector's OWN median, as a stated figure. Withholding the negotiation range is
  // justified by the reader keeping the facts — and for an address in a diverging
  // sector this is the most decision-relevant fact there is. Describing it as
  // "about 25% higher" and making the reader do the arithmetic is not keeping it.
  const sc = spine?.sectorContext;
  const sectorFigure =
    sc && sc.sectorMedian != null && sc.verdict !== "none"
      ? {
          sector: sc.sector,
          median: { raw: sc.sectorMedian, formatted: formatGBP(sc.sectorMedian) },
          count: sc.txCount ?? null,
          range:
            sc.ciLo != null && sc.ciHi != null
              ? { low: sc.ciLo, high: sc.ciHi, formatted: `${formatGBP(sc.ciLo)} – ${formatGBP(sc.ciHi)}` }
              : null,
          vsDistrict: sc.divergencePct == null ? null : { pct: sc.divergencePct, formatted: formatSignedPct(sc.divergencePct) },
        }
      : null;
  ...
  if (stats.summaryWithheld) {
    return {
      ...
      data: {
        // ALWAYS an object, never null. The client's market-overview grid
        // dereferences mo.latestYear/windowMedian/previousYear/confidence
        // unconditionally in six places; a null here white-screened every
        // below-floor brief. The shape is guaranteed HERE so no consumer needs a
        // guard — an optional chain downstream would convert the crash into a
        // silently missing section, which is worse.
        //
        // The figures are honestly absent (null → "—"); the COUNTS and the
        // confidence note are real, and summaryWithheld tells the client to render
        // the count-and-range variant instead of a grid of dashes.
        marketOverview: withheldOverview(stats, district),
...
/**
 * The market-overview block for a district with too few sales to state a median.
 * Same KEYS as the full block — that is the point — with the figures null and the
 * counts real. `summaryWithheld` carries the reason so the client can render the
 * count-and-range variant rather than a row of dashes.
 */
function withheldOverview(stats, district) {
  const dash = { raw: null, formatted: "—" };
  return {
    district,
    summaryWithheld: stats.summaryWithheld?.reason ?? "below-window-threshold",
    windowMedian: dash,
    latestYear: { year: stats.latestYear.year, count: stats.latestYear.count, median: dash },
    previousYear: { year: stats.previousYear.year, count: stats.previousYear.count, median: dash },
    yoyChange: { raw: null, formatted: "—", direction: "flat" },
    totalTransactions: stats.totalCount,
    confidence: { level: stats.confidence.level, note: stats.confidence.note },
  };
}
...
function buildMarketOverview(stats, district) {
  ...
  return {
    district,
    summaryWithheld: null, // present on BOTH shapes so they are structurally identical
