 ...
 */
export function buildPricesSection(stats, location, tier) {
  const base = {
...
function buildTrendBlock(stats) {
  const hasSparseYears = stats.trend.some((r) => r.state === "sparse" || r.state === "missing");
  ...
  return {
    ...
    rows,
    hasSparseYears,
    lowVolumeNote: hasSparseYears
      ? "Years marked “—” had no recorded sales; years with only a handful of sales give a volatile median — read the trend, not any single year."
      : null,
  };
}
