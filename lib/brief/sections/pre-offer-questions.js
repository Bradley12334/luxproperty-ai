 ...
 * @param {import("../tx-source.js").Spine|null} inputs.spine
 ...
 */
export function buildPreOfferQuestionsSection({ stats, spine, floodData, crimeData }, location, tier) {
  const base = {
  ...
  // Both sources carry these on the spine, so this no longer needs the raw
  // transaction set (which the aggregate path does not have) and no longer derives
  // a year from the clock.
  //   recentCount — sales in the trailing 3 calendar years, for volume signals
  //   recent      — the most recent individual sales, for COMPOSITION signals
  //                 (tenure mix, new-build share), which need records not a count
  const recentCount = spine?.recentWindowCount ?? 0;
  const recent = Array.isArray(spine?.recent) ? spine.recent : [];
  ...
  const newBuildShare = pct(newBuildCount, recentCount);
  if (newBuildShare != null && newBuildShare >= 30 && recentCount >= 6) {
    triggered.push({
      key: "newbuild",
  ...
  const state = spine || stats?.available ? "DATA" : "SPARSE";
