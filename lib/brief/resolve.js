 ...
 * @property {string|null} sector     Postcode sector, e.g. "E20 3" — the outcode plus the
 *   first character of the incode. NULL when the caller supplied a bare outcode: a
 *   district centroid has no single sector, and inventing one would attach
 *   sector-level claims to a district-wide request.
 *   Derived HERE and nowhere else. It was previously read as `location.sector` by
 *   generate.js against a field that did not exist, so `?? null` silently turned a
 *   missing property into "no sector" and the sector-divergence policy could never
 *   fire on any request.
...
/**
 * Postcode sector from a canonical full postcode: outcode + the first incode digit.
 * "E20 3BE" → "E20 3". Returns null for anything that is not a full postcode, so a
 * district-wide request can never acquire a sector by accident.
 * @param {string} canonical
 * @returns {string|null}
 */
export function sectorOf(canonical) {
...
async function resolveOutcode(outcode, raw) {
  ...
  return Object.freeze({
    ...
    outcode,
    // A district centroid has no single sector. Explicitly null rather than absent,
    // so a consumer reading it gets "there is no sector here" from a field that
    // demonstrably exists, not from a property that was never set.
    sector: null,
